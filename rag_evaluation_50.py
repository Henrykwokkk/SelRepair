...
checkpoint = "/starcoder2_rag_results/model_0.9/checkpoint"
...
with open ("./Tufano_dataset/50/src_source_code_unixAST_embedding_test_remaining_retrival.json",'r') as f1, open(
    ...
    for buggy_info, fixed_code in zip(buggy_codes,fixed_codes):
        ...
        inputs_ids = tokenizer.encode(inputs,max_length=550,truncation=True,return_tensors='pt').to(device)
        labels_ids = tokenizer.encode(fixed_code, max_length=120,truncation=True,return_tensors='pt').to(device)
        outputs = model.generate(inputs_ids,max_length=550)
